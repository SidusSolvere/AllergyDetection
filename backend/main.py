...
async def predict(file: UploadFile = File(...)):
    ...
    try:
        ...
        advice = generate_advice(cnn_result["result"], cnn_result["severity_percentage"])

        db = SessionLocal()
        db.add(Prediction(
            result=cnn_result["result"],
            severity_percentage=cnn_result["severity_percentage"],
        ))
        db.commit()
        db.close()

        return {**cnn_result, "advice": advice.model_dump()}
